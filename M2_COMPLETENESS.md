...
`TODO/TODO_correctness.md`) and not a historical record (that's git history).
...
| Animated material tint/fade (`M2Color`/`M2TextureWeight`, per-sequence or global-sequence-driven) | full | extras + Blender playback | extras-capped, permanent | no core-glTF animation-channel target for a material property exists — `resolveAnimatedColorCurve`/`resolveAnimatedFixed16Curve` (`src/cmd_export.cpp`, reusing `resolveVec3TrackSequence`/`resolveRawIntTrackSequence`) resolve the full curve and attach it as `tint_animation`/`fade_animation` material extras (`gltf::Material`); `tools/husk_blender_geoset_mask.py`'s `apply_tint_fade_animation` turns it into real playback post-import (frame-driven, looping on the curve's own real duration) — structurally consistent but **not verified against a real corpus fixture with actual tint/fade data**, see that function's own doc comment |
...
| Texture transform (UV scroll/rotate/scale, animated case) | full | extras + Blender playback | extras-capped, permanent | `KHR_texture_transform` has no animation-channel target, so the common case has no *native* glTF representation regardless of effort — but the real translation/rotation/scaling keyframe curve (whichever track is genuinely animated) is now resolved (`resolveAnimatedColorCurve`/`resolveAnimatedRawQuatCurve`, `src/export_texture_resolution.cpp`, the latter via new `resolveRawQuatTrackSequence`/`resolveRawQuatGlobalSequenceTrack`) and attached as `texture_transform_animation` material extras; `tools/husk_blender_geoset_mask.py`'s `apply_texture_transform_animation` turns it into real Mapping-node playback post-import — verified against the real `unk_exp11_7037014.m2` fixture (`tests/test_data_paths.hpp`'s `kTextureTransformTranslationM2`), see `TODO/TODO_correctness.md` #3 |
| Texture transform (UV scroll/rotate/scale, constant case) | full | native + extras | native — 100% | real `KHR_texture_transform` on `baseColorTexture` (`gltf_mesh.cpp`'s `textureTransformToKhr`) whenever the record is genuinely constant (every track either empty or a true single value, not just "every per-sequence value happens to be identity" — see the real `brewfestmount.m2` counterexample this session found) and the rotation is planar (Z-axis only); pivot-correction math (texture-center vs. the extension's origin) hand-derived and verified against real `bloodknightcharger.m2` data three independent ways (hand computation, 20,000 randomized trials against the client's own matrix composition, and Blender's own glTF importer producing an exactly-matching Mapping node) — see `TODO/TODO_correctness.md`'s former item 4, now closed. Raw values still always attached as `texture_transform` extras too, for the non-planar/non-real-constant fallback case |
...
| Player-housing collision (`PCOL`, War Within 11.1.7+) | full | diagnostic (`dump-chunks` JSON) | n/a, by design | wowdev.wiki M2#PCOL: four independent `(count, offset)` regions (`vertexPositions`/`faceNormals`/`indices`/`flags`), each read via its own offset since the wiki warns regions aren't necessarily contiguous — confirmed on a real file (an 8-byte gap between `faceNormals` and `indices`); verified against all 2,354 real `PCOL`-bearing files in the local corpus (corrected from an earlier scanner bug's false "zero real files," `WIKI_FINDINGS/M2.md`), every region in-bounds and every index in range for that file's own vertex count on all 2,354, `indexCount == faceNormCount * 3` on all 2,354 (triangle triples, one normal per triangle — the same shape M2's own core collision mesh above already has); `flags`' per-record meaning is undocumented on the wiki, exposed raw, but a full-corpus value scan confirmed it's structurally a real per-triangle bitmask, not an enum (98.4% of files use only bit 0; every value above 5 is a singleton confined to one specific doodad) — bit semantics unconfirmed, needing DB2/client data (`WIKI_FINDINGS/M2.md`); also found `flagsCount` isn't always equal to `faceNormCount` (3/2,354 real files differ), already handled correctly since `dumpPcol` reads it from its own header field; same `dump-chunks`-only, no-glTF-slot treatment as `EXP2`/`PFDC`/`DETL` (sidecar-shaped collision data, not core render geometry) |

## Interaction points & effects

| Feature | Parse | Consumption | glTF ceiling | Note |
|---|---|---|---|---|
| Attachments | full | native (position/joint) + extras (`animate_attached`) | native — 100% (placement); extras-capped, permanent (animated field) | real, translation-only child glTF node per entry (`attachment_<id>`), parented under its owning joint, never added to `skin.joints` — see `DESIGN.md`'s Key design decisions. `animate_attached` (M2Track<uint8_t>, "whether the attached model animates with this one, default true") is now resolved (reuses the same `resolveRawByteTrackCurve` Light's `visibility` uses) and attached as `animate_attached` node extras when non-default data exists — checked against 3 real fixtures (`wolf.m2`, `sword_2h_ashbringer_a_01.m2`, `mace_1h_warfrontsforsaken_d_01.m2`, 23 real attachments total) and found genuinely empty on all of them, a real negative result matching the wiki's own "only a bool is used, default is true" note, not a bug |
| Events | deref (static fields only) | native (`husk export`) | native — 100% | same treatment as attachments, node named `event_<identifier>` (not deduplicated — a real M2 can repeat one, e.g. multiple "$CSD" events); the raw `M2Event::data` field (wowdev.wiki: "passed when the event is fired," no per-event meaning documented) is attached as a `data` extras key on that same node |
| Lights | full | native (position/joint) + extras (`type` + animated curves) | native — 100% (placement); extras-capped, permanent (animated fields) | position/joint node named `light_<index>`, same as before; `type` and all 7 M2Track fields (ambient/diffuse color+intensity, attenuation start/end, visibility) are now resolved (`resolveLightColorCurve`/`resolveLightFloatCurve`/`resolveLightVisibilityCurve`, `cmd_export.cpp`) and attached as `type`/`light_animation` extras on the light's own node (`gltf_skeleton.cpp`) — same "no core-glTF animation-channel target for a light property exists" reasoning as the material tint/fade curves, reusing `gltf::Material::AnimatedColorCurve`/`AnimatedScalarCurve` rather than duplicating the shape. Verified against a real login-screen glue model (`ui_mainmenu_pandaria.m2`, `test_data/interface/glues/models/`) with 2 real animated lights — plausible in-range color/intensity/attenuation values, not garbage |
...
| Cameras (`M2Camera`) | descriptor (count only) | diagnostic (count only) | n/a, deprioritized by design | a custom renderer supplies its own camera regardless — see `TODO/TODO_correctness.md` #1 |
...
| `.bone` correction matrices (`BIDA`/`BOMT`) | full | diagnostic (`dump-chunks`) + inert `extras` (`husk export --bones-dir`/`--customization-choice-ids`) | extras-only, by design — never applied to the render | LOD ruled out as the selector by real data; slot selection is a real local `ChrCustomizationChoiceID` → `ChrCustomizationBoneSet.BoneFileDataID` DB2 chain, resolved via `husk export --db2-dir/--dbd-dir/--customization-choice-ids` (`src/chrcustomization_db2.hpp`/`.cpp`) — see `TODO/TODO_correctness.md` #2; applying the resolved matrix to real Blender rendering is the remaining open question, `TODO/BONE_CORRECTION_APPLICATION_TODO.md` |
| Lookup tables (`boneLookup`/`attachmentLookup`/`cameraLookup`/`textureLookup`/`sequenceLookup`) | deref | diagnostic | n/a — pure indirection metadata, no independent renderable shape | `husk info` now dereferences and prints all five, resolving names via the existing `keyBoneName`/`textureTypeName`/`attachmentTypeName` tables where the id has one, skipping 0xFFFF ("-1"/"unused") entries; `sequence_lookup` is printed as its real hash-bucket shape (wowdev.wiki M2#Animation_Lookup), not a direct id-index map. Verified against real `wolf.m2`/`bloodelffemale_hd.m2` data (key-bone entries resolve to the expected Head/Jaw/Root joints, texture-type entries match each model's own hardcoded texture slots). None of these feed `husk export` — they're pure indirection tables the renderer doesn't need (husk already exports full per-vertex global joint indices/real embedded textures instead of the batching schemes these tables exist for) |
