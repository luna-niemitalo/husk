...
| Collision / physics | 📖 `bounding_box`/`collision_box`/`collision_sphere_radius` scalars printed by `husk info`; the low-poly hit-test mesh's own content is now real, dereferenced data (`m2::CollisionMesh`/`parseCollisionMesh`, `src/m2.cpp`) — `husk export` writes it as a plain indexed triangle mesh in the `.glb` (one more `gltf::NamedMesh`, unskinned even when the render mesh shares a skeleton, per-vertex normals averaged from the M2's own per-face `collision_face_normals`), tagged `{"collision": true}` in glTF node `extras` (VERIFICATION_IDEAS.md case 5) so a renderer/Blender script can filter it out — not applied to any render/physics behavior itself, this tool has no runtime. `.phys` sidecar's `PFID` FileDataID is surfaced (`husk info`'s `phys_file_id`, same raw-ID-only treatment as `SKID`/`BFID`/`AFID` — see the Sidecar row below), but the `.phys` file's own *content* is still completely unparsed | ⬜ `M3CL` collision mesh (`CPOS`/`CNML`/`CINX`) | ⬜ `MOBN`/`MOBR` BSP tree, `MCVP` convex volumes, `MOPL` terrain-cutting planes | ⬛ |
...
- **Conformance** (`tests/test_conformance.cpp`) — a fourth tier, answering
  ...
  *third* leg: the M2 source file's own header counts (VERIFICATION_IDEAS.md
  cases 1/2/5) — `header.vertices.count`/`header.bones.count` against
  ...
  (case 3 — containment, not tight equality; see the test's own comment for
