...
| 12 | ~~`SwatchColor` DB2 availability~~ | `CHAR_TEXTURE_BLENDER_SWITCH_TODO.md` Still open | **Done 2026-08-22** — real, no new file needed: it's a column on `ChrCustomizationChoice.db2` itself, already opened by `chrcustomization_db2.cpp`. Genuine remaining gap: current layouts store it as a two-element array field, and `db2table::readNamedColumns` is scalar-only — needs a small array-column API before it can actually be read |
| 13 | ~~`live_gallery` curve playback, real-browser confirmation~~ | `TODO_correctness.md` #3 | **Done 2026-08-22** — playwright-mcp confirmed working after a session restart. Curve playback verified genuinely animating (not just static) via a rigorous `gl.readPixels` sum comparison across two samples a second apart, on a real 201-keyframe fixture. Found and fixed a real bug along the way (`viewer.js` never applied `blend_mode` extras, so additive materials rendered as opaque slabs) — full writeup in `TODO_correctness.md` #3. A second, deeper bug found during the same visual pass (wrong texture attached to a `weapon_blade` slot via over-eager fuzzy matching) is a real export-pipeline issue, not a viewer bug — logged separately as `TODO_correctness.md` #4, not fixed this session |
