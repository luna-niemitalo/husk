...
| `PIXEL_SHADER_FORMULAS_TODO.md` | Filling wowdev.wiki's 17 undocumented `Combiners_*` formulas | A promising but unverified lead found (`reference/wow.export`'s shader); step 1 (find real corpus repros) done 2026-08-14 — 14/17 have real repros; a full corpus-wide invariant/equivalence-testing pass (2026-08-20, `references/wow_shaders/combiner_hunt/`) verified 3 formulas exact, found 3 more real-but-ambiguous, 2 genuine negatives | Step 2 (final verification against real rendered output): human-gated |
| `COMBINER_HUNT_EXTENSIONS_TODO.md` | Two self-contained extensions to the combiner-hunt tooling: an external-weight-scalar search (unlocks `Dual_Crossfade`/`Masked_Dual_Crossfade`), a constant-output test tier (unlocks `Illum`) | Not started; fully self-contained, written to be picked up cold by a fresh session | Independent |
