...
| `CHAR_TEXTURE_BLENDER_SWITCH_TODO.md` | Blender-side live customization-choice texture switch (`CHAR_TEXTURE_COMPOSITING_TODO.md`'s Stage 5) — fully self-contained, written to be picked up cold by a fresh session | Not started; needs one small husk-side C++ addition (expose every choice per option, not just the resolved default) plus new `tools/husk_blender_geoset_mask.py` node-graph tooling | Independent |
