...
| `CHAR_TEXTURE_COMPOSITING_TODO.md` | Full DB2-driven character texture compositing (base/overlay skin layers) | **Closed, all 6 stages done** — kept as a stub, cited by name from `DESIGN.md`/`WIKI_FINDINGS*`/source doc comments; remaining work lives in `CHAR_TEXTURE_BLENDER_SWITCH_TODO.md` and `EQUIPPED_GEAR_RENDER_TODO.md` | N/A — nothing open |
...
| `EQUIPPED_GEAR_RENDER_TODO.md` | Blender-side rendering of `CHAR_TEXTURE_COMPOSITING_TODO.md`'s Stage 6 gear resolution (attach standalone-geometry items, texture-overlay object-skin items) | Scoped, not started — two structurally different sub-problems identified (attachment-point geometry vs. object-skin texture overlay reusing `CHAR_TEXTURE_BLENDER_SWITCH_TODO.md`'s own node-graph machinery); real design questions open (extras schema, slot->attachment mapping) | Independent to scope/implement; final visual pass is human-gated |
...
| `INVESTIGATIONS_TODO.md` | Meta-list of every open investigation-shaped item scattered across the files above, gathered in one place | New 2026-08-21 — 13 independent investigations catalogued, work started on 4 of them same session | Independent (that's the whole point of the list); a human-gated section is kept for completeness only |
