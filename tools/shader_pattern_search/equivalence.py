...
Formulas with `n_scalar > 0` (currently just Combiners_Mod_Dual_Crossfade /
_Masked_Dual_Crossfade) additionally search over `(cbuffer_name,
component_index)` candidates for their scalar roles -- these bind to an
external cbuffer scalar component (e.g. M2TextureWeight.g/.b) directly,
not to an already-assigned texture role's own alpha channel the way every
other formula here gets its blend factors "for free."

Known gaps (not implemented, not silently guessed at):
- Illum's real formula is "always black" -- a constant-output property,
  not a formula this equivalence machinery is shaped to test. See
  constant_output.py instead.
...
MAX_SCALAR_CANDIDATES = 8  # cap (cbuffer_name, component) candidate pairs tried per scalar role
...
def try_match(ir, slice_idxs: list[int], sink_name: str, sink_swizzle: str, formula: Formula,
    ...
    # (cbuffer_name, component_index) candidates for scalar roles (e.g.
    # M2TextureWeight.g/.b) -- distinct from const_perm above, which binds
    # a whole cbuffer free input's rgb as a vec3 tint (Guild family only).
    # A formula needing more scalar roles than there are candidates simply
    # can't match; capped per MAX_SCALAR_CANDIDATES's comment (combinatorial
    # cost -- see TODO/COMBINER_HUNT_EXTENSIONS_TODO.md item 1 step 4).
    scalar_candidates = [(name, comp) for name in cbuf_names for comp in range(4)][:MAX_SCALAR_CANDIDATES]
    if formula.n_scalar and len(scalar_candidates) < formula.n_scalar:
        return None
    scalar_perms = list(permutations(scalar_candidates, formula.n_scalar)) if formula.n_scalar else [()]
    ...
    for tex_perm in permutations(tex_names, formula.n_tex):
        for const_perm in permutations(cbuf_names, formula.n_const):
            for scalar_perm in scalar_perms:
                ...
                for component_order in permutations(range(3)):
                    ...
                    for t_i, assignments in enumerate(trial_assignments):
                        ...
                        scalar_vals = [assignments[name][comp] for name, comp in scalar_perm]
                        spec_out = formula.fn(tex_vals, const_vals, scalar_vals)
                        try:
                    ...
                    if ok:
                        return {"tex_roles": list(tex_perm), "const_roles": list(const_perm),
                                "scalar_roles": [f"{name}.{'xyzw'[comp]}" for name, comp in scalar_perm],
